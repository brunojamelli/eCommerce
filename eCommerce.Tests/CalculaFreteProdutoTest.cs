...
namespace eCommerce.Tests
{
    ...
    {
        [Fact]
        public void TestCalcularFrete_PesoMenorOuIgualA5()
        {
            // Pesos de produtos
            var prod01 = new Produto(1, "Biscoito Treloso de Chocolate", "Descrição do produto leve", 1.99m, 4.99m, TipoProduto.ALIMENTO);
            var prod02 = new Produto(2, "Televisão", "Descrição do produto no limite", 1000.00m, 5.00m, TipoProduto.ELETRONICO);

            // Verificações
            Assert.Equal(0, prod01.CalcularFrete());
            Assert.Equal(0, prod02.CalcularFrete());
        }
        [Fact]
